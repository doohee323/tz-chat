                    ...
                    .then(
                        function successCallback(res) {
                          if (res.data.userid) {
                            ...
                            $scope.roomid = $scope.target.chatroom;
                            ...
                            socket
                                .on(
                                    input.roomid + '_inserted',
                                    function(rslt) {
                                      if (rslt && rslt != ''
                                          && rslt != "{'status':'disconnect'}") {
                                        if (rslt === "{'status': 'closed'}") {
                                          $scope.close(null, true);
                                        } else {
                                          var target_image;
                                          if ($('#target_userid').val() == rslt.source) {
                                            target_image = $('#target_image')
                                                .val();
                                          } else {
                                            target_image = '[' + rslt.source
                                                + ']';
                                          }
                                          gf_receive({
                                            image : target_image,
                            ...
                            $scope.sendMsg = function(scope, key) {
                              if (scope.text1 && scope.text1 != '') {
                                if ($scope.roomid.indexOf('chatroom') == -1) {
                                  ...
                                  point = point - 50;
                                  if (point < 0) {
                                    sweetAlert('', 'Point is not enough.',
                                        'error');
                                    return;
                                  }
                                ...
                                socket.emit(input.roomid + '_insert', msg);
                                scope.text1 = '';
                              }
                            };
...
var gf_receive = function(data, reload) {
  ...
  if (data.image.indexOf('[') == 0) {
    obj.find(".c-name2").text(data.image)
  }
